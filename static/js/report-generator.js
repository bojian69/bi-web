...
const ReportGenerator = {
    // 生成完整的数据分析报表
    generateFullReport: function(data, options = {}) {
        if (!data || !data.columns || !data.rows || data.rows.length === 0) {
            return '<div class="error-message">无有效数据可分析</div>';
        }
        
        const selectedDimensions = options.dimensions || ['overview', 'statistics', 'quality', 'distribution', 'outliers'];
        ...
        html += '<div class="report-header">'
            + '<h2><i class="fas fa-file-alt"></i> 数据分析报表</h2>'
            + '<div class="report-timestamp">生成时间: ' + new Date().toLocaleString() + '</div>'
            + '</div>';
        
        // 添加数据概览
        if (selectedDimensions.includes('overview')) {
            html += DataAnalysisRules.reportRules.generateOverviewReport(data);
        }
        
        // 添加统计分析
        if (selectedDimensions.includes('statistics')) {
            html += this.generateStatisticsReport(data);
        }
        
        // 添加数据质量分析
        if (selectedDimensions.includes('quality')) {
            html += this.generateQualityReport(data);
        }
        
        // 添加分布分析
        if (selectedDimensions.includes('distribution')) {
            html += this.generateDistributionReport(data);
        }
        
        // 添加异常值分析
        if (selectedDimensions.includes('outliers')) {
            html += this.generateOutliersReport(data);
        }
        
        // 添加结论和建议
        html += this.generateInsightsAndRecommendations(data);
    ...
    // 生成统计分析报表
    generateStatisticsReport: function(data) {
        if (!data || !data.columns || !data.rows || data.rows.length === 0) {
            return '';
        }
        ...
        data.columns.forEach((column, colIndex) => {
            ...
            if (DataAnalysisRules.typeDetection.isNumeric(colValues)) {
                numericColumns.push({
                    name: column,
                    index: colIndex,
                    values: colValues.map(val => typeof val === 'string' ? parseFloat(val.replace(/,/g, '')) : parseFloat(val))
        ...
        if (numericColumns.length === 0) {
            html += '<p>未发现数值型列，无法进行统计分析</p>';
        } else {
            html += '<table class="analysis-table" style="width: 100%; border-collapse: collapse;">'
                ...
                + '<th style="padding: 8px; text-align: left; border-bottom: 1px solid #ddd;">平均值</th>'
                + '<th style="padding: 8px; text-align: left; border-bottom: 1px solid #ddd;">中位数</th>'
                + '<th style="padding: 8px; text-align: left; border-bottom: 1px solid #ddd;">最小值</th>'
                + '<th style="padding: 8px; text-align: left; border-bottom: 1px solid #ddd;">最大值</th>'
                + '<th style="padding: 8px; text-align: left; border-bottom: 1px solid #ddd;">标准差</th>'
                + '</tr>'
                + '</thead><tbody>';
            
            numericColumns.forEach(column => {
                const stats = DataAnalysisRules.comparisonRules.calculateBasicStats(column.values);
                
                html += '<tr>'
                    + '<td style="padding: 8px; border-bottom: 1px solid #eee;">' + column.name + '</td>'
                    + '<td style="padding: 8px; border-bottom: 1px solid #eee;">' + stats.count + '</td>'
                    + '<td style="padding: 8px; border-bottom: 1px solid #eee;">' + stats.mean.toFixed(2) + '</td>'
                    + '<td style="padding: 8px; border-bottom: 1px solid #eee;">' + stats.median.toFixed(2) + '</td>'
                    + '<td style="padding: 8px; border-bottom: 1px solid #eee;">' + stats.min.toFixed(2) + '</td>'
                    + '<td style="padding: 8px; border-bottom: 1px solid #eee;">' + stats.max.toFixed(2) + '</td>'
                    + '<td style="padding: 8px; border-bottom: 1px solid #eee;">' + stats.stdDev.toFixed(2) + '</td>'
                    + '</tr>';
            });
